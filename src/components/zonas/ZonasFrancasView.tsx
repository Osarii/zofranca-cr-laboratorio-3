...
export const ZonasFrancasView: React.FC<ZonasFrancasViewProps> = ({ zonasFrancas, onCreate }) => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200">
      <div><p className="mb-1 text-xs font-extrabold uppercase tracking-[0.18em] text-[#9A4D5D]">RF-01 · Configuración</p><h1 className="text-[28px] font-extrabold tracking-tight text-[#5A1F2D]">Zonas francas y criterios mínimos</h1><p className="mt-1 text-sm text-[#6B5A52]">Registre los parámetros que utilizará el motor para medir afinidad empresarial.</p></div>

      <div className="grid gap-6 lg:grid-cols-5">
        <form onSubmit={guardar} className="space-y-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm lg:col-span-2">
          <div className="flex items-center gap-3 border-b border-slate-100 pb-4"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-sky-50 text-[#9A4D5D]"><Plus className="h-5 w-5" /></span><div><h2 className="font-extrabold text-[#5A1F2D]">Registrar zona franca</h2><p className="text-xs text-slate-500">Todos los criterios son obligatorios.</p></div></div>
          ...
          <button type="submit" disabled={guardando} className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#9A4D5D] px-5 py-3 text-xs font-bold text-white hover:bg-[#7C3545] disabled:opacity-60">{guardando ? <span className="h-4 w-4 animate-spin rounded-full border-2 border-white border-t-transparent" /> : <Save className="h-4 w-4" />}{guardando ? 'Guardando…' : 'Guardar zona franca'}</button>
        </form>

        <section className="space-y-4 lg:col-span-3">
          <div className="flex items-center justify-between"><h2 className="font-extrabold text-[#5A1F2D]">Zonas registradas</h2><span className="rounded-full bg-slate-200 px-3 py-1 text-xs font-bold text-slate-600">{zonasFrancas.length}</span></div>
          {zonasFrancas.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-slate-300 bg-white py-16 text-center"><MapPinned className="mx-auto h-10 w-10 text-slate-300" /><p className="mt-3 text-sm font-bold text-[#5A1F2D]">Aún no hay zonas registradas</p></div>
          ) : zonasFrancas.map((zona) => (
            <article key={zona.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              <div className="flex items-start gap-3"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-sky-50 text-[#9A4D5D]"><Building2 className="h-5 w-5" /></span><div className="min-w-0 flex-1"><div className="flex items-center justify-between gap-3"><h3 className="font-extrabold text-[#5A1F2D]">{zona.nombre}</h3><span className="font-mono text-[11px] font-bold text-slate-400">ZF-{zona.id}</span></div><div className="mt-3 grid gap-3 text-xs sm:grid-cols-2"><span className="flex items-center gap-2 text-slate-600"><DollarSign className="h-4 w-4 text-emerald-600" />Mínimo: <strong className="text-slate-800">${zona.inversionMinima.toLocaleString('en-US')}</strong></span><span className="flex items-center gap-2 text-slate-600"><Users className="h-4 w-4 text-[#9A4D5D]" />Mínimo: <strong className="text-slate-800">{zona.empleosMinimos} empleos</strong></span></div><div className="mt-4 flex flex-wrap gap-2">{zona.sectoresPermitidos.map((sector) => <span key={sector} className="rounded-md bg-slate-100 px-2 py-1 text-[11px] font-semibold text-slate-700">{sector}</span>)}</div></div></div>
            </article>
          ))}
        </section>
      </div>
...
const Campo: React.FC<{ etiqueta: string; id: string; children: React.ReactNode }> = ({ etiqueta, id, children }) => (
  <div><label htmlFor={id} className="mb-1.5 block text-xs font-bold text-[#5A1F2D]">{etiqueta}</label>{children}</div>
);
