...
export const PDFExportModal: React.FC<PDFProps> = ({ solicitud, item, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#5A1F2D]/60 backdrop-blur-xs animate-in fade-in">
      <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
        {/* Header */}
        <div className="px-6 py-4 bg-[#5A1F2D] text-white flex items-center justify-between">
          <div className="flex items-center space-x-2">
            <ShieldCheck className="w-5 h-5 text-[#9A4D5D]" />
        ...
        <div className="p-8 overflow-y-auto space-y-6 text-slate-800 text-xs bg-slate-50/50">
          <div className="border-b-2 border-[#5A1F2D] pb-4 flex justify-between items-start">
            <div>
              <div className="text-xl font-extrabold text-[#5A1F2D]">PROCOMER / COMEX</div>
              <div className="text-[11px] text-[#6B5A52] font-semibold">República de Costa Rica — Régimen de Zonas Francas</div>
            </div>
            <div className="text-right font-mono text-[11px] text-[#6B5A52]">
          ...
          <div>
            <h3 className="text-sm font-bold text-[#5A1F2D] uppercase tracking-wide">
              Certificado de Dictamen Técnico de Viabilidad
            </h3>
            <p className="text-[#6B5A52] mt-1 leading-relaxed text-xs">
          ...
          <div className="bg-white p-4 rounded-xl border border-slate-200 grid grid-cols-2 gap-3 shadow-xs">
            <div>
              <span className="font-bold text-[#6B5A52] block text-[11px] uppercase">Puntaje Viabilidad IA:</span>
              <span className="text-base font-extrabold text-[#9A4D5D] flex items-center gap-1">
                <Cpu className="w-4 h-4" /> {score} / 100
              </span>
            </div>
            <div>
              <span className="font-bold text-[#6B5A52] block text-[11px] uppercase">Estado Dictamen:</span>
              <span className="text-base font-extrabold text-[#5A1F2D]">{status}</span>
            </div>
            <div>
              <span className="font-bold text-[#6B5A52] block text-[11px] uppercase">Inversión Comprometida:</span>
              <span className="text-slate-900 font-bold text-xs">USD ${(investment).toLocaleString()}</span>
            </div>
            <div>
              <span className="font-bold text-[#6B5A52] block text-[11px] uppercase">Empleos Directos:</span>
              <span className="text-slate-900 font-bold text-xs">{jobs} Plazas Formales</span>
            </div>
          </div>

          <div className="text-[11px] text-[#6B5A52] leading-relaxed bg-white p-3.5 rounded-xl border border-slate-200">
          ...
          <div className="pt-6 border-t border-slate-200 flex justify-between items-end text-center">
            <div className="w-44 border-t border-slate-400 pt-1 text-[10px] text-[#6B5A52] font-bold">
        ...
        <div className="px-6 py-3.5 bg-slate-100 border-t border-slate-200 flex justify-end gap-2">
          ...
          <button 
            onClick={onClose} 
            className="px-5 py-2 bg-[#9A4D5D] hover:bg-[#7C3545] active:bg-[#713044] text-white font-bold text-xs rounded-xl shadow-xs hover:shadow-md transition-all duration-150 flex items-center gap-1.5 cursor-pointer"
          >
