        ...
        {abierto && (
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div><p className="font-extrabold text-[#0B2B4A]">Centro de sonido</p><p className="mt-0.5 text-[11px] text-slate-500">Probá cada efecto inmediatamente.</p></div>
            ...
            <label className="mt-4 block rounded-xl bg-slate-50 p-3">
              <span className="flex items-center justify-between text-xs font-bold text-[#0B2B4A]"><span className="flex items-center gap-1.5"><SlidersHorizontal className="h-3.5 w-3.5" />Volumen</span><span>{Math.round(volumen * 100)}%</span></span>
              <input aria-label="Volumen de los sonidos" type="range" min="0" max="1" step="0.05" value={volumen} onChange={(evento) => cambiarVolumen(Number(evento.target.value))} className="mt-2 w-full accent-[#2D9CDB]" />
            </label>
