...
export const PersonaAHeader: React.FC<PersonaAHeaderProps> = ({
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-sky-300/10 bg-[#0B2B4A]/95 text-white shadow-[0_12px_30px_-18px_rgba(2,15,30,.9)] backdrop-blur-xl">
      <div className="h-0.5 bg-gradient-to-r from-transparent via-[#2D9CDB] to-transparent opacity-80" />
      <div className="mx-auto max-w-[1500px] px-4 sm:px-6 lg:px-8">
        <div className="flex h-16 items-center justify-between gap-3">
          <button type="button" onClick={() => setCurrentTab('solicitudes')} className="group flex shrink-0 items-center gap-2.5 rounded-xl py-1 text-left" aria-label="Ir a solicitudes">
            <motion.span whileHover={{ rotate: -4, scale: 1.06 }} whileTap={{ scale: 0.94 }} className="relative flex shrink-0 drop-shadow-lg"><ZoFrancaLogo className="h-10 w-10" /><span className="absolute -bottom-0.5 -right-0.5 h-3 w-3 rounded-full border-2 border-[#0B2B4A] bg-emerald-400" /></motion.span>
            <span className="hidden sm:block">
              <span className="block text-lg font-black leading-none tracking-tight text-white">ZoFranca</span>
              <span className="mt-1 block text-[9px] font-extrabold uppercase tracking-[0.22em] text-sky-300">Costa Rica</span>
            </span>
          </button>
          ...
          <nav className="hidden min-w-0 items-center gap-1 rounded-xl border border-white/10 bg-black/10 p-1 shadow-inner min-[1450px]:flex" aria-label="Navegación principal">
            {opciones.map(({ id, label, icono: Icono, badge }) => (
              <button type="button" key={id} onClick={() => setCurrentTab(id)} className={`relative flex items-center gap-1.5 whitespace-nowrap rounded-lg px-2.5 py-2 text-xs font-semibold transition ${currentTab === id ? 'text-white' : 'text-slate-300 hover:bg-white/10 hover:text-white'}`}>
                {currentTab === id && <motion.span layoutId="navegacion-activa" className="absolute inset-0 rounded-lg border border-sky-300/20 bg-gradient-to-b from-white/20 to-white/10 shadow-sm" transition={{ type: 'spring', stiffness: 420, damping: 34 }} />}
                <span className="relative flex items-center gap-1.5"><Icono className={`h-4 w-4 ${currentTab === id ? 'text-[#2D9CDB]' : ''}`} />{label}</span>
        ...
        <div className="relative -mx-4 border-t border-white/10 sm:-mx-6 min-[1450px]:hidden">
          <div className={`pointer-events-none absolute inset-y-0 left-0 z-10 w-16 bg-gradient-to-r from-[#0B2B4A] via-[#0B2B4A]/90 to-transparent transition-opacity ${limitesScroll.inicio ? 'opacity-0' : 'opacity-100'}`} />
          <div className={`pointer-events-none absolute inset-y-0 right-0 z-10 w-16 bg-gradient-to-l from-[#0B2B4A] via-[#0B2B4A]/90 to-transparent transition-opacity ${limitesScroll.fin ? 'opacity-0' : 'opacity-100'}`} />
          <button type="button" onClick={() => desplazarNavegacion(-1)} disabled={limitesScroll.inicio} aria-label="Ver opciones anteriores" className={`absolute left-1 top-1/2 z-20 flex h-7 w-7 -translate-y-1/2 items-center justify-center rounded-full border border-white/15 bg-[#123a5c] text-white shadow-lg transition ${limitesScroll.inicio ? 'pointer-events-none opacity-0' : 'opacity-100 hover:bg-[#1a4d74]'}`}><ChevronLeft className="h-4 w-4" /></button>
          <nav ref={navegacionMovilRef} className="navigation-scroll flex snap-x snap-mandatory gap-1 overflow-x-auto px-10 py-2 sm:px-12" aria-label="Navegación móvil">
            {opciones.map(({ id, label, icono: Icono, badge }) => (
              ...
              >
                {currentTab === id && <motion.span layoutId="navegacion-movil-activa" className="absolute inset-0 rounded-xl border border-sky-300/20 bg-gradient-to-r from-[#2D9CDB]/35 to-emerald-400/15" transition={{ type: 'spring', stiffness: 380, damping: 32 }} />}
                <span className="relative flex items-center gap-1.5"><Icono className={`h-3.5 w-3.5 ${currentTab === id ? 'text-sky-300' : ''}`} />{label}</span>
                {!!badge && <span className="relative rounded-full bg-rose-500 px-1.5 text-[9px] text-white">{badge}</span>}
              </motion.button>
            ))}
          </nav>
          <button type="button" onClick={() => desplazarNavegacion(1)} disabled={limitesScroll.fin} aria-label="Ver más opciones" className={`absolute right-1 top-1/2 z-20 flex h-7 w-7 -translate-y-1/2 items-center justify-center rounded-full border border-white/15 bg-[#123a5c] text-white shadow-lg transition ${limitesScroll.fin ? 'pointer-events-none opacity-0' : 'opacity-100 hover:bg-[#1a4d74]'}`}><ChevronRight className="h-4 w-4" /></button>
        </div>
      </div>
    </header>
  );
