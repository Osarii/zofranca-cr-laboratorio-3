...
export const NewSolicitudModal: React.FC<NewSolicitudModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-[#0B2B4A]/65 p-4 backdrop-blur-sm">
      <div className="my-6 w-full max-w-2xl overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
        <div className="flex items-center justify-between bg-[#0B2B4A] px-6 py-4 text-white">
          <div className="flex items-center gap-3">
            <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-[#2D9CDB]">
        ...
        <form onSubmit={enviar} className="space-y-5 p-6">
          ...
          <div>
            <label htmlFor="empresa" className="mb-1.5 flex items-center gap-2 text-xs font-bold text-[#0B2B4A]">
              <Building2 className="h-4 w-4 text-[#2D9CDB]" /> Empresa o razón social
            </label>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label htmlFor="zona" className="mb-1.5 flex items-center gap-2 text-xs font-bold text-[#0B2B4A]">
                <MapPin className="h-4 w-4 text-[#2D9CDB]" /> Zona franca
              </label>
            ...
            <div>
              <label htmlFor="sector" className="mb-1.5 block text-xs font-bold text-[#0B2B4A]">Sector productivo</label>
          ...
          {zonaSeleccionada && (
            <div className="grid gap-3 rounded-xl border border-sky-100 bg-sky-50/70 p-4 text-xs sm:grid-cols-3">
              <div><span className="block text-slate-500">Inversión mínima</span><strong className="text-[#0B2B4A]">${zonaSeleccionada.inversionMinima.toLocaleString('en-US')}</strong></div>
              <div><span className="block text-slate-500">Empleos mínimos</span><strong className="text-[#0B2B4A]">{zonaSeleccionada.empleosMinimos}</strong></div>
              <div><span className="block text-slate-500">Sectores habilitados</span><strong className="text-[#0B2B4A]">{zonaSeleccionada.sectoresPermitidos.length}</strong></div>
            </div>
          )}

          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label htmlFor="inversion" className="mb-1.5 flex items-center gap-2 text-xs font-bold text-[#0B2B4A]"><DollarSign className="h-4 w-4 text-[#2D9CDB]" /> Inversión proyectada (USD)</label>
            ...
            <div>
              <label htmlFor="empleos" className="mb-1.5 flex items-center gap-2 text-xs font-bold text-[#0B2B4A]"><Users className="h-4 w-4 text-[#2D9CDB]" /> Empleos proyectados</label>
          ...
          <div className="flex flex-col-reverse gap-3 border-t border-slate-100 pt-5 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex justify-end gap-2">
              ...
              <button type="submit" disabled={guardando || zonasFrancas.length === 0} className="flex items-center gap-2 rounded-xl bg-[#2D9CDB] px-5 py-2.5 text-xs font-bold text-white shadow-sm hover:bg-[#2387be] disabled:cursor-not-allowed disabled:opacity-50">
